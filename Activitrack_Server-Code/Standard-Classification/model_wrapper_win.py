...
class ModelWrapper:
	...
	def build_windowed_df(self, df, features_to_expand):
		fun_to_apply_td = {
		...
		dictDF = {}
		dictDF['count'] = df['accelerometer_x'].count()

		dictDF = ({col + '_' + fname : [f(df[col])] for col in features_to_expand for fname, f in fun_to_apply_td.items()})
	...
	def predict_csv(self, csv_file):
		...
		ts = df.timestamp
		df_w = self.build_windowed_df(df, feat)
		prediction =  clf.predict(df_w)
