...
print('Loading data ...')
data1 = pd.read_csv('/home/kakadinh/adria/UCI HAR Dataset2/Phones_accelerometer.csv')		#Loading Accelerometer data
data2 = pd.read_csv('/home/kakadinh/adria/UCI HAR Dataset2/Phones_gyroscope.csv')			#Loading Gyroscope data

length1 = len(data1)
length2 = len(data2)
length = min(length1, length2)						#To make the length of the merged data equal to minimum of the two data 
# length = round(0.7*length)
data1 = data1.drop(labels = ['Arrival_Time','Creation_Time','Index', 'User','Model','Device'], axis=1)		#Dropping the unnecessary fields
...
model.save("LSTM_UCI_Dataset2.h5")	
